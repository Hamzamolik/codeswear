...
const Checkout = ({ cart, AddToCart, RemoveFromCart, subTotal }) => {
  ...
  const iniatepayment = async () => {
    ...
    let config = {
      ...
      "handler": {
        "notifymerchant": function handler(eventname, data) {
  ...
  return (
    <div className='m-auto container'>
      ...
      <Script src={`${process.env.HOST}/merchantpgpui/checkoutjs/merchants/${process.env.MID}.js`} type="application/javascript" crossOrigin='anonymous' onLoad="onscriptload();" />
      <div className="text-3xl font-bold my-8 text-center">Check Out</div>
      <h2 >1. Delivery Details</h2>
      <div className="m-auto flex">
        <div className="px-2 w-1/2">
          <div className="relative mb-4">
            <label htmlFor="name" className='leading-7 text-sm text-gray-600 '>Your Name</label>
            <input id="name" name="name" onChange={setdis} value={name} type="text" autoComplete="email" required className="w-full rounded border bg-white border-gray-200  focus:border-pink-500 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" placeholder="Your name" />
          </div>
        </div>
        <div className="px-2 w-1/2">
          <div className="relative mb-4">
            <label htmlFor="name" className='leading-7 text-sm text-gray-600 '>Your email</label>
            <input id="name" name="email" onChange={setdis} value={email} type="email" autoComplete="email" required className="w-full rounded border bg-white border-gray-200  focus:border-pink-500 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" placeholder="Your email" />
          </div>
        </div>
      </div>
      <div className="px-2 ">
      ...
      <div className="mx-auto flex my-2">
        <div className="px-2 w-1/2">
          <div className="mb-4">
            ...
            <input type="text" id="state" onChange={setdis} value={state} name="state" className="w-full bg-white rounded border border-gray-300 focus:border-pink-500 focus:ring-2 focus:ring-pink-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" readOnly />
          </div>
        </div>
        <div className="px-2 w-1/2">
          <div className="mb-4">
            <label htmlFor="district" className="leading-7 text-sm text-gray-600 dark:text-gray-400">District</label>
            <input type="text" id="district" onChange={setdis} value={city} name="district" className="w-full bg-white rounded border border-gray-300 focus:border-pink-500 focus:ring-2 focus:ring-pink-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" readOnly />
          </div>
        </div>
      </div>
      ...
      <div className="w-[100%]  sidebar justify-center md:justify-start  overflow-hidden  px-5 py-10 rounded-xl bg-pink-100 my-2">
        ...
        <button onClick={iniatepayment} disabled={edisabled} className="flex mr-2 disabled:bg-pink-300 text-white bg-pink-500 border-0 p-2 focus:outline-none hover:bg-pink-600 rounded text-sm"> pay : ₹ {subTotal} </button>
      </div>
